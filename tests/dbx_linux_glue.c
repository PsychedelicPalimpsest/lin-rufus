/*
 * dbx_linux_glue.c — build shim for test_dbx_linux.
 *
 * Provides stubs for everything net.c needs except:
 *   - settings (ReadSetting64/WriteSetting64) — supplied by real common/parser.c
 *   - uprintf/vuprintf/uprintfs — supplied by real stdio.c
 *   - stdfn helpers — supplied by real stdfn.c
 *
 * The real common/parser.c provides get_token_data_file / set_token_data_file,
 * which are required by the settings inline helpers in settings.h.
 */
...
/* ---- efi_archname (defined in iso.c; referenced by net.c/hash.c) ---- */
...
/* NotificationEx: always decline (IDNO) so CheckForDBXUpdates skips downloads */
...
/* ---- ValidateOpensslSignature (test key stub — never actually called) ---- */
