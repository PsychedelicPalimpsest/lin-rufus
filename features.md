...
- **Keyboard shortcuts** ✅ **DONE**: Added `Alt+J` (toggle Joliet), `Alt+K` (toggle Rock Ridge), `Alt+F` (toggle USB HDD detection + refresh device list). These match the Windows keyboard shortcuts that affect ISO parsing and device enumeration behaviour. NOTE: `UNATTEND_USE_MS2023_BOOTLOADERS` in `ApplyWindowsCustomization` only logs on Linux — the actual boot.wim EFI replacement (EFI_EX/Fonts_EX extraction) is not yet implemented; tracked as a future item.
...
125. End to end testing of mocked UI/CLI to ensure functionality (with emulated devices and `/dev/nvme0n2` for the specific linux box you are on)
