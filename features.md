...
74. 🔧 **Volume-label enforcement per filesystem** — `GetWindowTextA(hLabel,...)` now works via `window_text_bridge` so the user-typed label flows from GTK to FormatThread. FAT32 boot sector `sVolLab` is now correctly populated (uppercase, space-padded, 11 chars) from the Label parameter — 5 new tests pass. Still needed: call `e2label`/`exfatlabel`/`ntfslabel`/`udflabel` post-format for ext2/3/4, exFAT, NTFS, UDF filesystems
