...
121. ~~**Use docker to allow for non root root style testing**~~ ✅ **DONE** — `run_tests.sh --container` builds and runs root-requiring tests (loopback, block-device) inside a privileged Docker/Podman container (ubuntu:22.04 base, mirrors GitHub CI); `--full-container` runs the complete suite; `tests/Dockerfile` defines the image; `tests/install-deps.sh` is the single source of truth for apt packages used by both Dockerfile and `.github/workflows/linux.yml`; rootless podman handled via `podman image scp` into root's store + `sudo podman run --privileged`.
...
122. ✅ **Settings dialog** — `on_settings_clicked()` connected to the ⚙ toolbar button in `ui_gtk.c`; shows a `GtkDialog` with two sections: (1) **Update Checks** — three GtkRadioButtons (daily/weekly/disabled) read/write `SETTING_UPDATE_INTERVAL`; (2) **Behaviour** — five GtkCheckButtons for dark mode (`SETTING_DARK_MODE`, applies via `gtk-application-prefer-dark-theme`), expert mode (`SETTING_EXPERT_MODE`), USB debug logging (`SETTING_ENABLE_USB_DEBUG`), detect fake flash drives (`SETTING_DISABLE_FAKE_DRIVES_CHECK` inverted), ignore boot marker (`SETTING_IGNORE_BOOT_MARKER`); all settings are read on open and saved to the INI file on OK; changes take effect immediately. Mirrors the Windows "Application Options" update-policy dialog (`IDD_UPDATE_POLICY`/`IDC_SETTINGS`).
...
180. ~~**Write test for the full UI, running with a mock xorg or wayland server, and using automation software (or equivlent) to test the actual button presses the user would do**~~ ✅ **DONE** — `tests/test_ui_automation_linux.c` + `tests/rufus_ui_automation.py`. 10 automated AT-SPI2 UI tests run rufus under Xvfb+DBus: widget tree traversal (Device label), status label (READY), START button exists, CLOSE button exists+enabled, Settings dialog opens (xdotool verifies "Application Settings" X11 window), Ctrl+L log dialog, Ctrl+P persistent log toggle, advanced drive properties toggle (state change verified via AT-SPI2 STATE_CHECKED), Quick format checkbox toggle, and close-button-exits-app (AT-SPI2 doAction → g_application_quit). Bug fixed: `on_close_clicked()` was calling `gtk_main_quit()` which has no effect when using `g_application_run()` — replaced with `g_application_quit(G_APPLICATION(gtk_window_get_application(...)))`. All 10 UI automation tests pass; full test suite: 81 passed, 0 failed.
---
...
140. ~~**VHD/VHDX DD write support in `linux/format.c`**~~ ✅ **DONE** — When `write_as_image=TRUE` and `boot_type=BT_IMAGE` and the selected image is a VHD or VHDX (`img_report.compression_type == IMG_COMPRESSION_VHD/VHDX`), `FormatThread` in `src/linux/format.c` now mounts the image via `VhdMountImageAndGetSize()` (qemu-nbd) before writing, redirects `image_path` to the NBD device, copies via `format_linux_write_drive()`, then unmounts via `VhdUnmountImage()` (called unconditionally in the `out:` label). Write-verify is skipped for VHD/VHDX images (source is no longer accessible post-unmount). `format_linux_write_drive()` gained block-device size detection (`BLKGETSIZE64` ioctl via `S_ISBLK` check) so it correctly reads the full NBD device rather than treating `st_size=0` as empty. Three qemu-nbd improvements in `src/linux/vhd.c`: (1) explicit `--format=vpc`/`--format=vhdx` passed to avoid ambiguous-format exit-1; (2) post-connect size poll retries up to 3 s for the device to become ready; (3) 200 ms `sync()` + sleep after disconnect so the kernel fully releases the device before re-connect. Linked into all format-using test suites (`test_format_linux`, `test_format_thread_linux`, `test_loopback`, `test_e2e_linux`, `test_qemu_boot_linux`, `test_real_device_linux`) with appropriate glue stubs (`ignore_boot_marker`, `has_ffu_support`, `fd_md5sum`, `nb_blocks`, `HashFile`, `wuprintf`) and WIM/CDIO/XML dependencies (`--start-group/--end-group`). 11 tests in `tests/test_vhd_format_linux.c` (7 unit + 2 root integration) all pass; full `--linux-only` suite: all tests pass.

141. ~~**KolibriOS boot support in `linux/format.c`**~~ ✅ **DONE** — Feature parity with Windows `format.c`: (1) `format_linux_write_mbr()` now checks `HAS_KOLIBRIOS(img_report) && IS_FAT(fs_type)` in the default case and calls `write_kolibrios_mbr()` (first byte = `0x33`); (2) ISO extraction section installs the KolibriOS USB loader: `ExtractISOFile(image_path, "HD_Load/USB_Boot/MTLD_F32", "<mount>/MTLD_F32", 0)` with a warning logged on failure. `write_kolibrios_mbr` is in `src/ms-sys/br.c` (shared with Windows). `ExtractISOFile` stub added to `format_linux_glue.c`, `loopback_linux_glue.c`, and `format_thread_linux_glue.c` (the last one tracks calls for test assertions). 3 new tests in `test_format_thread_linux.c`: `write_mbr_image_kolibrios_fat_writes_kolibri_code` (first byte = 0x33), `write_mbr_image_kolibrios_ntfs_does_not_write_kolibri_code` (NTFS uses default MBR — boot codes differ), `format_thread_kolibrios_installs_loader_via_extractisofile` (FormatThread calls ExtractISOFile with "HD_Load/USB_Boot/MTLD_F32"). All 197 format_thread tests pass; full `--linux-only` suite: all tests pass.
142. ~~**`UpdateMD5Sum` call in Linux format thread**~~ ✅ **DONE** — Feature parity with Windows `format.c`: after ISO extraction, `FormatThread` in `src/linux/format.c` calls `UpdateMD5Sum(mount_path, md5sum_name[...])` when `!IS_ERROR(ErrorStatus) && !windows_to_go`. `extern` declarations for `md5sum_name[2]` and `UpdateMD5Sum` added to `format.c`. `UpdateMD5Sum` stubs added to `format_thread_linux_glue.c` (tracks `update_md5sum_call_count`), `format_linux_glue.c`, and `loopback_linux_glue.c`. 2 new tests: `format_thread_iso_calls_update_md5sum` (has_md5sum=1 → called) and `format_thread_wintogo_skips_update_md5sum` (WTG → not called). 201 format_thread tests pass; full `--linux-only` suite: all tests pass.
143. ~~**Win7 EFI boot setup + `CopySKUSiPolicy` in Linux format thread**~~ ✅ **DONE** — Feature parity with Windows `format.c`: (1) After KolibriOS loader installation, when `(TT_UEFI || allow_dual_uefi_bios) && HAS_WIN7_EFI(img_report)` (has_efi==1 && wininst_index>0), `FormatThread` creates `<mount>/efi/boot/` and calls `WimExtractFile(image_path|sources/install.wim, 1, "Windows/Boot/EFI/bootmgfw.efi", "<mount>/efi/boot/bootx64.efi")`. On Linux, `wininst_path[0]` stores a `/`-prefixed relative ISO path; the WIM path is `image_path + "|" + &wininst_path[0][1]`. (2) After Win7 EFI block, `CopySKUSiPolicy(mount_path)` is called unconditionally (no-op stub on Linux). `extern` declarations for `allow_dual_uefi_bios` and `CopySKUSiPolicy` added to `format.c`. `--wrap=WimExtractFile` and `--wrap=GetExtPartitionName` added to format_thread LDFLAGS to allow stub injection. `g_test_mount_path` global in `format_thread_linux_glue.c` lets Win7 EFI test inject a real tmpdir so `mkdir` succeeds. `CopySKUSiPolicy` stubs added to `format_linux_glue.c` and `loopback_linux_glue.c`. 3 new tests: `format_thread_win7_efi_uefi_calls_wimextractfile`, `format_thread_non_win7_efi_skips_wimextractfile_for_efi`, `format_thread_iso_calls_copy_sku_si_policy`. 208 format_thread tests pass; full `--linux-only` suite: all tests pass.
144. ~~**WinPE masquerading in Linux format thread**~~ ✅ **DONE** — Feature parity with Windows `format.c`: when `HAS_WINPE(img_report) && !img_report.uses_minint`, `FormatThread` sets `needs_masquerading=TRUE`. Two effects: (1) After `CreatePartition`, the boot indicator byte at MBR offset 446 is patched from `0x80` to `0x81` via `pwrite(fd, &indicator, 1, 446)` (WinPE masquerade marker). (2) `format_linux_write_mbr(hPhysicalDrive, needs_masquerading)` uses Rufus MBR instead of Win7 MBR when `use_rufus_mbr || needs_masquerading`. Signature of `format_linux_write_mbr` updated to accept `BOOL needs_masquerading` parameter (declaration in `format_linux.h`, implementation in `format.c`, all call sites updated). 3 new tests: `format_thread_winpe_nonminint_sets_masquerade_boot_indicator` (0x81 written), `format_thread_winpe_minint_does_not_masquerade` (0x80 kept), `format_thread_winpe_nonminint_uses_rufus_mbr` (Rufus MBR first byte = 0x41). 217 format_thread tests pass; full `--linux-only` suite: all tests pass.
145. ~~**ZIP archive extraction in Linux format thread**~~ ✅ **DONE** — Feature parity with Windows `format.c`: after `OP_FINALIZE`, when `!IS_ERROR(ErrorStatus) && archive_path != NULL && fs_type < FS_EXT2`, `FormatThread` mounts the main partition via `AltMountVolume(DriveIndex, part_offset, FALSE)`, calls `ExtractZip(archive_path, zip_mount)`, and unmounts. On Linux we mount/unmount explicitly since there is no persistent drive letter. `archive_path` is already defined in `globals.c`; `ExtractZip` is in `stdio.c` (declared in `windows/rufus.h`). `extern char *archive_path` declaration added to `format.c`. `ExtractZip` stubs added to `format_thread_linux_glue.c` (tracks `extract_zip_call_count` and `extract_zip_last_dst`), `format_linux_glue.c`, and `loopback_linux_glue.c`. 3 new tests: `format_thread_extracts_zip_when_archive_path_set` (archive_path non-NULL + FAT32 → called), `format_thread_skips_zip_when_archive_path_null` (NULL → not called), `format_thread_skips_zip_for_ext_fs` (ext2 filesystem → not called). 223 format_thread tests pass; full `--linux-only` suite: all tests pass.
146. ~~**FAT32 VBR variant selection in WritePBR_fs (ReactOS, KolibriOS, WinPE, BOOTMGR)**~~ ✅ **DONE** — Feature parity with Windows `format.c`: `WritePBR_fs` in `src/linux/format.c` now selects the correct VBR writer based on boot type and `img_report` flags: `BT_FREEDOS`→`write_fat_32_fd_br`, `BT_REACTOS`→`write_fat_32_ros_br`, `BT_IMAGE+HAS_KOLIBRIOS`→`write_fat_32_kos_br`, `BT_IMAGE+HAS_BOOTMGR`→`write_fat_32_pe_br`, `BT_IMAGE+HAS_WINPE`→`write_fat_32_nt_br`, default→`write_fat_32_br`. Tests: `write_pbr_fat32_generic_passes`, `write_pbr_fat32_reactos_matches_ros_vbr`, `write_pbr_fat32_bootmgr_matches_pe_vbr`, `write_pbr_fat32_winpe_matches_nt_vbr`, `write_pbr_fat32_kolibrios_matches_kos_vbr` — all verify with `entire_fat_32_*_br_matches()`. Test helper `create_fat32_sector()` creates a 16 KiB file with FAT32 signature in both primary (sector 0) and backup (sector 6) to handle both VBR iterations. 241 format_thread tests pass; full `--linux-only` suite: all tests pass.
147. ~~**AnalyzeMBR diagnostic + WriteSBR (GRUB4DOS secondary boot sectors)**~~ ✅ **DONE** — Feature parity with Windows `format.c`: (1) Before `ClearMBRGPT`, `FormatThread` calls `AnalyzeMBR(hPhysicalDrive, "Drive", FALSE)` + `UpdateProgress(OP_ANALYZE_MBR, -1.0f)` to log the existing MBR type. (2) New `format_linux_write_sbr(hDrive)` function writes the secondary sectors of `grldr.mbr` (bytes 512–8191) at disk offset 0x200 for `BT_GRUB4DOS` (and `BT_IMAGE+has_grub4dos`). This is required for GRUB4DOS to chain-load from BIOS; without it the sector-0 stub code cannot find the GRUB4DOS loader. The 7680-byte secondary block is embedded as `src/ms-sys/inc/mbr_grub_secondary.h` (auto-generated from `res/grub/grldr.mbr[512:]`). For `BT_GRUB2`: no-op (grub-install writes core.img). Called in `FormatThread` immediately after `format_linux_write_mbr`. `format_linux_write_sbr` is declared in `format_linux.h`. 5 new tests: `write_sbr_bad_handle_fails`, `write_sbr_non_bootable_is_noop`, `write_sbr_grub4dos_writes_secondary_block`, `write_sbr_grub2_is_noop`, `write_sbr_image_grub4dos_writes_secondary_block`. 262 format_thread tests pass; full `--linux-only` suite: all tests pass.
148. ~~**XP_COMPAT BIOS compatibility partition in `CreatePartition` + `FormatThread`**~~ ✅ **DONE** — Feature parity with Windows `format.c`/`drive.c`: (1) `IDC_OLD_BIOS_FIXES` checkbox now sets `XP_COMPAT` in `extra_partitions` inside `FormatThread` (`src/linux/format.c`): `if (IsChecked(IDC_OLD_BIOS_FIXES)) extra_partitions |= XP_COMPAT;`. (2) `CreatePartition()` in `src/linux/drive.c` extended for `XP_COMPAT`: when `extra_partitions & XP_COMPAT`, shrinks the main partition by `compat_sects` (= `SelectedDrive.SectorsPerTrack` if > 0, else 63 — one traditional track) and writes an MBR type-`0x00` placeholder entry at `total_sects - compat_sects`. Main partition ends exactly where compat partition starts (no gap). 5 new tests in `test_partition_ops_linux.c`: `create_partition_mbr_with_compat_shrinks_main`, `create_partition_mbr_with_compat_partition_at_end`, `create_partition_mbr_with_compat_partition_size`, `create_partition_mbr_with_compat_custom_track_size`, `create_partition_mbr_with_compat_main_contiguous`. 77 partition_ops tests pass; full `--linux-only` suite: all tests pass.
149. ~~**NTFS fixup via `ntfsfix` (RunNtfsFix) in `FormatThread`**~~ ✅ **DONE** — Feature parity with Windows `format.c` `CheckDisk()`: after `OP_FINALIZE`, when `boot_type==BT_IMAGE && img_report.is_iso && fs_type==FS_NTFS`, `FormatThread` calls `GetExtPartitionName()` to get the partition device path, then `RunNtfsFix(partition_path)`. `RunNtfsFix()` lives in its own file `src/linux/ntfsfix.c` (separate from `format.c` so linker `--wrap` can intercept it in tests) and runs `ntfsfix "<partition_path>"` via `system()`. Declared in `src/linux/format_linux.h`. Stubs added to all test glue files (`format_thread_linux_glue.c`, `loopback_linux_glue.c`, `badblocks_integration_glue.c`, `e2e_linux_glue.c`). 4 new tests: `format_thread_iso_ntfs_calls_ntfsfix`, `format_thread_iso_ntfs_passes_partition_path`, `format_thread_non_ntfs_iso_skips_ntfsfix`, `format_thread_non_iso_ntfs_skips_ntfsfix`, `format_thread_non_image_ntfs_skips_ntfsfix`. 270 format_thread tests pass; full `--linux-only` suite: all tests pass.
150. ~~**UEFI:NTFS BIOS exclusion for Windows installer images + Syslinux HAS_WINDOWS condition**~~ ✅ **DONE** — Two parity gaps fixed: (1) `FormatThread` now excludes the UEFI:NTFS extra partition for Windows images targeting BIOS-only (no `allow_dual_uefi_bios`): after `extra_partitions |= XP_UEFI_NTFS`, clears the flag when `BT_IMAGE && HAS_BOOTMGR_BIOS && !windows_to_go && TT_BIOS && !allow_dual_uefi_bios`. Mirrors Windows `format.c` comment: "only if we're not dealing with a Windows image in installer mode with target system set to BIOS and without dual BIOS+UEFI boot enabled." (2) Syslinux install condition updated to include `(!HAS_WINDOWS(img_report) || !allow_dual_uefi_bios)` — when a Windows image is flashed with `allow_dual_uefi_bios`, Syslinux is skipped. Mirrors Windows `format.c` line 1806. 4 new tests: `format_thread_windows_bios_ntfs_no_uefi_ntfs_partition`, `format_thread_windows_bios_allow_dual_adds_uefi_ntfs`, `format_thread_windows_uefi_target_adds_uefi_ntfs`, `format_thread_windows_dual_bios_uefi_skips_syslinux`. 286 format_thread tests pass.
151. ~~**Syslinux/Isolinux version detection in Linux `iso.c`**~~ ✅ **DONE** — Critical parity gap: `img_report.sl_version` was always 0 on Linux because `isolinux_path` was destroyed without reading any `.bin` files. Now, after the `config_path` block in `ExtractISO` scan mode, each `isolinux.bin` in `isolinux_path` is read via `ReadISOFileToBuffer` (ISO-relative path = strip `psz_extract_dir` prefix) and passed to `GetSyslinuxVersion`. On success: `sl_version`, `sl_version_ext`, and `sl_version_str` are set. On failure: falls back to `embedded_sl_version[has_ldlinux_c32 ? 1 : 0]` (same as Windows). `GetSyslinuxVersion` moved to its own file `src/linux/sl_version.c` (no heavy deps) so it can be linked into both `syslinux.c` and the ISO tests without pulling in libinstaller/libfat/block-device symbols. Makefile: `sl_version.c` added to `SYSLINUX_LINUX_SRC` and `EXTRA_SRC_LINUX_test_iso_linux`; `embedded_sl_version[2]` stub added to `iso_linux_glue.c`. 3 new tests in `test_iso_linux.c`: `extract_iso_scan_detects_syslinux_version` (sl_version == 0x0603), `extract_iso_scan_detects_syslinux_version_str` (sl_version_str == "6.03"), `extract_iso_no_syslinux_version_is_zero` (non-Syslinux ISO → sl_version stays 0). 12413 ISO tests pass.

152. ~~**Post-scan detections in Linux `iso.c`**~~ ✅ **DONE** — Implemented GRUB2 version detection (reads `boot/grub[2]/i386-pc/normal.mod` via `GetGrubVersion`), EFI boot entry post-processing (`GetGrubVersion`+`GetGrubFs` for EBT_GRUB, `GetEfiBootInfo` for EBT_MAIN), and `has_grub2_fs` bitmask computation from `grub_filesystems` StrArray. 3 new tests in `test_iso_linux.c` (12418 total pass). Commit `b19ea8e2`.

153. ~~**Post-extraction steps in Linux `iso.c` (extraction mode parity)**~~ ✅ **DONE** — (a) Solus/Mint broken UEFI bootloader workaround ✅: when `has_efi & 0xc000`, deletes `<dest_dir>/EFI/boot/bootx64.efi` if `& 0x4000` (borked symlink case, e.g. Mint), then calls `DumpFatDir(dest_dir, 0)` to extract EFI files from the FAT efi.img. `DumpFatDir` + `iso9660_readfat` moved to `src/linux/dump_fat.c` (separate TU) so `--wrap=DumpFatDir` works in tests; `__wrap_DumpFatDir` tracks calls in `iso_linux_glue.c`. (b) OpenSUSE config_path + `needs_syslinux_overwrite` ✅. (c) syslinux.cfg creation ✅. (d) Knoppix `symlinked_syslinux` workaround ✅: during extraction, Rock Ridge symlink `syslinux → isolinux` is detected and recorded; after extraction, the symlink is replaced with a real directory and `syslnx32.cfg`/`syslnx64.cfg` redirectors are created. 9 new tests (6 EFI/Knoppix + 3 infrastructure), 12440 total pass.

154. ~~**Post-scan parity gaps in Linux `iso.c` (has_efi=0x8000, wininst_version)**~~ ✅ **DONE** — (a) `has_efi = 0x8000` set post-scan when `!IS_EFI_BOOTABLE && HAS_EFI_IMG && HasEfiImgBootLoaders()` (Solus-style: EFI only in FAT efi.img). Mirrors Windows iso.c line 1049. (b) `wininst_version = GetWimVersion("iso_path|/sources/install.wim")` called when `HAS_WININST`. `GetWimVersion` intercepted via `--wrap=GetWimVersion` in iso test. 7 new tests (4 logic + 2 integration + 1 negative), 12447 total pass. `extern uint32_t GetWimVersion` declared in iso.c.

155. ~~**Scan-time Rock Ridge parity gaps in Linux `iso.c`**~~ ✅ **DONE** — (a) `has_deep_directories = TRUE` + r=-1 early return when Rock Ridge PL field (`ISO_ROCK_SUF_PL`) is detected during scan. Mirrors Windows iso.c lines 605-620. (b) `needs_ntfs = TRUE` when `live → casper` RR symlink found during scan (Mint LMDE). Mirrors Windows iso.c lines 666-669. 3 new tests (logic inline + Mint LMDE ISO fixture), 12451 total pass.

156. ✅ DONE **`SetFSFromISO` needs_ntfs parity: prefer NTFS when `needs_ntfs=TRUE`** — The Linux `SetFSFromISO` in `ui_combo_logic.c` doesn't check `IS_FAT32_COMPAT(img_report)` (which includes `needs_ntfs`). When `needs_ntfs=TRUE` (e.g. Mint LMDE), NTFS should be the strongly preferred filesystem, mirroring Windows rufus.c lines 193-209 where only NTFS is added to the FS combo for non-FAT32-compatible images.
...
175. ✅ DONE **Fast-zeroing (`fast_zeroing` flag) in `format_linux_write_drive()`** —
    - **Parity gap**: Windows `WriteDrive()` supports two zeroing modes: regular zero (fill with 0x00) and fast-zero (fill with 0xFF, skip blocks already empty to reduce flash wear). The Linux zeroing code always wrote 0x00 and never used the `fast_zeroing` global even though the Alt+Ctrl+Z keyboard shortcut (item 139) already sets it.
    - **Fix**: Rewrote `bZeroDrive` path in `format_linux_write_drive()`: (1) `memset(buf, fast_zeroing ? 0xFF : 0x00, chunk)` — fills with appropriate byte. (2) When `fast_zeroing=TRUE`: allocates a second comparison buffer; for each chunk, calls `pread` to read the current content; if all 4-byte words match 0x00000000 or 0xFFFFFFFF (already empty/erased), skips `pwrite` entirely; otherwise writes 0xFF and sets throttle=15 (skips reads for next 15 iterations to bound read overhead). Progress uses MSG_306 ("Fast-zeroing drive: %s") vs MSG_286 ("Zeroing drive: %s"). (3) Regular zero: no comparison buffer, no throttle, writes 0x00 directly.
    - **Tests**: 5 new tests in `tests/test_write_drive_linux.c`: `zero_drive_fills_with_zeros` (0x00 result), `fast_zero_drive_fills_with_0xff` (0xAA pre-fill → 0xFF result), `fast_zero_skips_already_ff_blocks` (0xFF pre-fill → skip → still 0xFF), `fast_zero_skips_already_zero_blocks` (0x00 pre-fill → skip → still 0x00), `fast_zero_overwrites_nonempty_blocks` (0x55 pre-fill → write → 0xFF result). 41016 checks all pass.

176. ✅ DONE **Hardware write-protect detection via sysfs `/sys/block/<dev>/ro`** —
    - **Parity gap**: Windows `FormatThread` detects write-protected media via `FCC_MEDIA_WRITE_PROTECTED` callback and reports `ErrorStatus = RUFUS_ERROR(ERROR_WRITE_PROTECT)` + "Media is write protected". The Linux `FormatThread` only detected `INVALID_HANDLE_VALUE` from `GetPhysicalHandle` (which reports generic `ERROR_OPEN_FAILED`) with no specific write-protect message.
    - **Fix**: Added `IsDeviceWriteProtected(DWORD DriveIndex)` to `src/linux/drive.c`. It extracts the block device name from the drive entry's `id` field (stripping the `/dev/` prefix), then reads `{sysfs_root}/block/<devname>/ro`. Returns TRUE if non-zero, FALSE on any error (no sysfs entry, invalid index, etc.) — safe default is "not protected". Added `g_drive_sysfs_root` static variable (default `/sys`), overridable in `RUFUS_TEST` builds via `drive_linux_set_sysfs_root()`. `FormatThread` now checks `IsDeviceWriteProtected` immediately after DriveIndex validation, before any of the three write paths (zero-drive, write-image, format). Also updated `GetDriveTypeFromIndex` to use `g_drive_sysfs_root` instead of hardcoded `/sys`.
    - **Tests**: 6 new tests in `tests/test_drive_linux.c`: `write_protect_invalid_drive_returns_false`, `write_protect_no_sysfs_file_returns_false`, `write_protect_ro_zero_returns_false`, `write_protect_ro_one_returns_true`, `write_protect_nvme_path_extracts_name`, `write_protect_sda_path_extracts_name`. All 106 drive tests pass (100 existing + 6 new).

177. ✅ DONE **Write retry logic + correct progress op in raw copy path** —
    - **Parity gap 1 (retries)**: Windows `WriteDrive()` uses `WRITE_RETRIES=4` retry loops with `Sleep(200*i)` between attempts for all write operations. The Linux raw copy path in `format_linux_write_drive()` had no retry logic — a single transient USB bus error would abort the entire write.
    - **Fix**: Added retry loop (`for try = 0..WRITE_RETRIES`) with `usleep(200000 * try)` between attempts around `pwrite()` in the raw copy path. Uses `pwrite` (position-independent) so no seek-back needed between retries. Breaks early on `IS_ERROR(ErrorStatus)` to respect user cancel.
    - **Parity gap 2 (progress op)**: Raw copy was calling `UpdateProgressWithInfo(OP_FILE_COPY, ...)` instead of `OP_FORMAT`. Windows uses `OP_FORMAT` for drive writes (`update_progress()` helper in Windows format.c). This means the Linux progress bar was advancing on the wrong operation phase.
    - **Fix**: Changed `OP_FILE_COPY` → `OP_FORMAT` in `UpdateProgressWithInfo` call in the raw copy loop.
    - **Tests**: Verified by existing raw copy tests in `test_write_drive_linux.c` (41016 checks pass).

178. ✅ DONE **Sector-aligned writes for compressed images (bled callback)** —
    - **Parity gap**: Windows `WriteDrive()` passes a `sector_write()` callback to `bled_init()` that buffers data into a static `sec_buf[4096]` and only calls `WriteFile` on full-sector boundaries. This prevents partial-sector writes to block devices. The Linux bled path was passing `NULL` (no write callback), causing bled to call `write()` directly with arbitrary-sized chunks that could misalign to sector boundaries.
    - **Fix**: Added `sector_write()` static function to `src/linux/format.c`. Logic mirrors Windows: (1) fast path for already-aligned writes from a fresh buffer position; (2) fills existing partial buffer, flushes on full sector; (3) writes remaining full sectors; (4) buffers remaining partial-sector tail. Uses `write(2)` directly (no `_write` macro, avoids compat header dependency in test builds). Updated `bled_init()` call to pass `sector_write` instead of `NULL`. Reset `sec_buf_pos = 0` before each bled call. Added post-bled flush: if `sec_buf_pos > 0` after `bled_uncompress_with_handles` returns (non-error), writes one full sector to device (padding remainder with whatever was in the buffer — same as Windows).
    - **Tests**: 1 new test `write_drive_gz_non_sector_aligned` in `tests/test_write_drive_linux.c`: creates a 500-byte raw pattern (`i & 0xFF`) gzip-compressed, calls `format_linux_write_drive`, verifies all 500 bytes read back correctly from the destination fd. 41022 checks all pass.

179. ✅ DONE **`large_drive` detection in Linux `FormatThread`** —
    - **Parity gap**: Windows `format.c` line 1387 sets `large_drive = (SelectedDrive.DiskSize > (1*TB))` and logs a notice early in `FormatThread`. Linux never set this global; it remained `FALSE` unconditionally. The flag is used by `ms-sys/file.c` to suppress short-write errors on drives >1 TB, so leaving it `FALSE` could mask real errors on large drives.
    - **Fix**: Added `large_drive = (SelectedDrive.DiskSize > (1*TB))` with `uprintf` notice in all three code paths inside Linux `FormatThread`: zero-drive, write-as-image, and normal format — each immediately after their `GetDrivePartitionData` call.
    - **Tests**: 4 new tests in `tests/test_format_thread_linux.c`: `format_thread_large_drive_sets_flag` (2 TB mock → TRUE), `format_thread_small_drive_clears_flag` (real 512 MB → FALSE), `format_thread_large_drive_boundary_exactly_1tb` (exactly 1 TB → FALSE), `format_thread_large_drive_boundary_1tb_plus_1` (1 TB + 1 byte → TRUE). Uses new `g_mock_disk_size_override` in `format_thread_linux_glue.c` + `--wrap=GetDrivePartitionData` linker wrap. 300 passed, 0 failed.

180. ✅ DONE **`write_as_esp` partition type parity in `CreatePartition`** —
