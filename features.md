...
153. **Post-extraction steps in Linux `iso.c` (extraction mode parity)** — The `else` branch of Linux `ExtractISO` (non-scan mode) only closes fd_md5sum and frees md5sum_data. Windows does 4 additional things after extraction:
    - **(a) Broken UEFI bootloader workaround** (`has_efi & 0xc000`): For Solus/Mint ISOs where `bootx64.efi` is a broken symlink, delete it and call `DumpFatDir(dest_dir, 0)` to extract from FAT `efi.img`. Linux has `DumpFatDir` but it's never called here.
    - **(b) OpenSUSE config_path fix + `needs_syslinux_overwrite`**: Windows config_path selection (scan mode) prioritizes `/boot/[i386|x86_64]/loader/isolinux.cfg` and sets `needs_syslinux_overwrite=TRUE`. Linux just picks the shortest path. Additionally the Tails "at equal length, always pick the latest" logic uses `>=` while Linux uses `<`.
    - **(c) syslinux.cfg creation** (CRITICAL): After extraction, if `HAS_SYSLINUX(img_report)`, create `<dest_dir>/syslinux.cfg` with `DEFAULT loadconfig + LABEL loadconfig + CONFIG <cfg_path>`. If `needs_syslinux_overwrite` and `syslinux.cfg` already exists, rename it to `syslinux.org` first. Without this, Syslinux-based distros won't boot from USB!
    - **(d) Knoppix symlinked_syslinux workaround**: If `symlinked_syslinux[0] != 0`, delete the symlink, create the directory, and create `syslnx32.cfg` / `syslnx64.cfg` files pointing to the isolinux directory.
    Plan: Write tests for (b) config_path selection and (c) syslinux.cfg creation first (these are testable in scan-test mode), implement, then handle (a) and (d) in extraction-mode tests.
