...
155. **Scan-time Rock Ridge parity gaps in Linux `iso.c`** — Two missing scan-time detections vs Windows iso.c: (a) `has_deep_directories = TRUE` when Rock Ridge PL field (`ISO_ROCK_SUF_PL`) is detected during scan; also cuts scanning short (r=-1) for performance, identical to Windows iso.c lines 605-620. (b) `needs_ntfs = TRUE` when a symlink `live → casper` is found (Mint LMDE requires NTFS for working symlinks); mirrors Windows iso.c lines 666-669. Both fields are used by common code (`iso_report.c` lines 80, 120) and `IS_FAT32_COMPAT` macro.
