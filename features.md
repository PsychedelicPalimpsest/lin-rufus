...
126. **`IsRevokedBySvn()` proper UTF-16 handling** — currently stubbed in `src/linux/hash.c` because Linux `wchar_t` is 4 bytes while PE SBAT strings use 2-byte UTF-16 code units; apply the same `wchar16_to_utf8()` helper already used in `iso.c` (or extract it to `common/`) so SBAT SVN revocation checks work correctly; add ≥ 5 tests in `test_hash.c` covering ASCII and multi-byte SVN version strings
...
128. **`DownloadToFileOrBufferEx()` GTK progress dialog integration** — `hDlg` is silently discarded with a `(void)hDlg; /* TODO */` comment in `src/linux/net.c`; implement: when `hDlg != NULL`, post `UM_DOWNLOAD_PROGRESS` messages via `PostMessage(hDlg, UM_DOWNLOAD_PROGRESS, percent, 0)` from the libcurl progress callback; wire a handler in `main_dialog_handler` that updates the GTK progress bar; add 3 tests in `test_net_linux.c` verifying the progress callback fires and posts the expected message type
...
132. **`CreateStaticFont()` / `SetHyperLinkFont()` → Pango markup hyperlinks** — render clickable URLs that appear in the log widget or status label using `<a href="…">` Pango markup on `GtkLabel`; connect `activate-link` signal to `g_app_info_launch_default_for_uri()`; expose `set_hyperlink_label(GtkWidget*, const char* url, const char* text)` helper in `ui_gtk.c`; covers the 🟡 stubs in section 3j and the pending item 41; add 4 tests verifying markup is generated correctly and that `xdg-open` is called
...
136. **Phase 3 common merger: portable logic in `format.c`, `drive.c`, `iso.c`** — audit the three largest source files for platform-independent logic that can move to `common/`; candidates: `GetExtFsLabel()` string normalisation in `drive.c`, `sanitize_label()` (already in `common/parser.c`), cluster-size validation table in `format.c`, ISO 9660 magic-byte constants in `iso.c`; each extraction must be accompanied by matching tests and must not break existing Windows or Linux builds
